...
def render_lerf_set(model_path, name, iteration, views, gaussians,  lerf_model, pipeline, background):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        dino_feature, clip_feature = render_pkg["feature_dinomap_precomp"], render_pkg["feature_clipmap_precomp"]
        if idx == 0:
            assert dino_feature.shape[0] == gaussians.get_xyz.shape[0], f"the dino feature shape {dino_feature.shape} and the gaussian shape {gaussians.get_xyz.shape[0]} are not matched!"
            assert clip_feature.shape[0] == gaussians.get_xyz.shape[0], f"the clip feature shape {clip_feature.shape} and the gaussian shape {gaussians.get_xyz.shape[0]} are not matched!"
            assert clip_feature.shape[0] == dino_feature.shape[0], f"the clip feature shape {clip_feature.shape} and the dino feature shape {dino_feature.shape} are not matched!"
            np.save(os.path.join(model_path, 'dino_feature_gsplat.npy'), dino_feature.cpu().numpy())
            np.save(os.path.join(model_path, 'clip_feature_gsplat.npy'), clip_feature.cpu().numpy())
            print("The dino feature and clip feature are saved in the model path!")
            return
